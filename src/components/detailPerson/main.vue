...
<script type="text/ecmascript-6">
    ...
    export default {
        data() {
            return {
                /*默认头像*/
                defaultPic: '//img.58cdn.com.cn/olympia/img/house/detail/renzheng_manHead.png?platform=app',
                contacts: {
                    /*微聊*/
                    chat: {
                        text: '微聊',
        ...
        computed: {
            user(){
                ...
                if (_this.dataUser.pic == null || _this.dataUser.pic == '') {
                    _this.dataUser.pic = _this.defaultPic;
                }
                Object.assign(_this.dataUser, {roleName: _roleName});
                return _this.dataUser;
            }
        },
        methods: {
            /*进入房东资料页*/
            handlerUserDetail(event){
                ...
                let _this = this;
                let _url = window.location.protocol + API.userDetail.url + '?platform=app&id=' + _this.dataUser.userid;
                this.$app.loadPage('link', _url, '', 0, 0, 0);
            },
            /*调起微聊*/
            handlerChat(event){
                this.$emit('clickChat', event);
                let _user = this.dataUser;
